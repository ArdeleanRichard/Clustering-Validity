            ...
            if len(np.unique(labels_clustering)) == 1:
                ...
                if clusterer_name not in clusterer_stats:
                    clusterer_stats[clusterer_name] = {
                        ...
                        'cvi': {m: [] for m in cvis}, 'cvi_nn': {m: [] for m in cvis}
                    }
                continue
...
def main_analysis_per_dataset(data_type):
    ...
    if data_type == "real_data":
        cvis.remove("CDbw")  # cannot construct hull # Failed to compute CDbw: QH6214 qhull input error: not enough points to construct initial simplex
...
if __name__ == "__main__":
    # main_analysis_data_per_dataset(data_type="synth_data")
