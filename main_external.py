...
def balanced_ari(labels_true, labels_pred, method='macro'):
    """
    ...
    balanced_ari = method_balance(method=method, class_scores=class_scores, class_sizes=class_sizes)
...
def balanced_cost_sensitive(cvi, labels_true, labels_pred, cost_matrix=None):
    """
    Cost-sensitive balanced metric.
    Weights class pairs by misclassification costs.
    ...
    labels_pred : array-like
        Predicted labels
    cost_matrix : array-like of shape (n_classes, n_classes) or None
        Cost of confusing class i with class j
        If None, uses uniform costs (equivalent to macro averaging)

    Returns:
    --------
    cost_sensitive_score : float
    """
    labels_true = np.asarray(labels_true)
    labels_pred = np.asarray(labels_pred)
    ...
    # Compute weighted per-class scores
    class_scores = []
    class_weights = []
    ...
    for i, c in enumerate(classes):
        ...
        # Weight by average cost for this class
        class_cost = np.mean(cost_matrix[i, :])

        class_scores.append(score)
        class_weights.append(class_cost)
...
if __name__ == "__main__":
    ...
    print(f"Sklearn implementation: {adjusted_rand_index_python(y_true, y_pred):.6f}")
    ...
    print()
    print(f"Custom implementation: {balanced_ari(y_true, y_pred):.6f}")
    print(f"Custom implementation: {balanced_external(adjusted_rand_index_numpy, y_true, y_pred):.6f}")
    print(f"Custom implementation: {balanced_external(adjusted_rand_score, y_true, y_pred, method='macro'):.6f}")
    print(f"Custom implementation: {balanced_external(adjusted_rand_score, y_true, y_pred, method='weighted'):.6f}")
    print(f"Custom implementation: {balanced_external(adjusted_rand_score, y_true, y_pred, method='sqrt_weighted'):.6f}")
    print(f"Custom implementation: {balanced_external(adjusted_rand_score, y_true, y_pred, method='harmonic'):.6f}")
    print(f"Custom implementation: {balanced_sample_weighted(adjusted_rand_score, y_true, y_pred):.6f}")
    print(f"Custom implementation: {balanced_stratified_subsample(adjusted_rand_score, y_true, y_pred):.6f}")
    print(f"Custom implementation: {balanced_pairwise_classes(adjusted_rand_score, y_true, y_pred):.6f}")
    print(f"Custom implementation: {balanced_minmax(adjusted_rand_score, y_true, y_pred):.6f}")
    print(f"Custom implementation: {balanced_cost_sensitive(adjusted_rand_score, y_true, y_pred):.6f}")

    print("\nComparison with sklearn:")
    print(f"Sklearn implementation: {adjusted_rand_index_python(y_true, y_pred):.6f}")
    print()
    print(f"Custom implementation: {balanced_external(adjusted_mutual_info_score, y_true, y_pred, method='macro'):.6f}")
    print(f"Custom implementation: {balanced_external(adjusted_mutual_info_score, y_true, y_pred, method='weighted'):.6f}")
    print(f"Custom implementation: {balanced_external(adjusted_mutual_info_score, y_true, y_pred, method='sqrt_weighted'):.6f}")
    print(f"Custom implementation: {balanced_external(adjusted_mutual_info_score, y_true, y_pred, method='harmonic'):.6f}")
