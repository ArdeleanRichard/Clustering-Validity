...
def create_error_scatter_plot(X, error_cases, best_ari_case, metric, dataset_name, algo_name, lower_is_better, output_folder):
    ...
    if n_cases <= 3:
        ...
        n_cols = 4
        n_rows = ceil(n_cases / 4)
    elif n_cases <= 15:
        n_cols = 5
        n_rows = ceil(n_cases / 5)
    else:
        n_cols = 6
        n_rows = ceil(n_cases / 6)
    ...
    for idx, (ax, case) in enumerate(zip(axes, all_cases)):
        ...
        ax.set_title(f"{title_prefix} ARI={ari:.4f}, {metric}={cvi[metric]}", fontsize=9, fontweight='bold' if idx == 0 else 'normal')
