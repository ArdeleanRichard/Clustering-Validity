...
from constants import FOLDER_FIGS_DATA, scale, FOLDER_RESULTS_LABELSETS
...
def main_synthetic_data_with_labelsets(plot=False):
    ...
    for data_name, (X, gt) in datasets:
        ...
        create_indices_table_with_arrows(X, label_sets=label_sets, save=f"{FOLDER_RESULTS_LABELSETS}/CVIs_{data_name}.csv", prnt=True)
...
def main_summarize():
    import pandas as pd
    import glob
    from constants import FOLDER_RESULTS_LABELSETS
    FOLDER = FOLDER_RESULTS_LABELSETS + "/*.csv"

    cvi_correct_counts = {}  # +1 if entire row has ZERO *
    cvi_error_counts = {}  # +1 for each * in any cell of that row
    ...
    for file in glob.glob(FOLDER):
        ...
        for metric in df.index:
            ...
            cvi_correct_counts.setdefault(metric, 0)
            cvi_error_counts.setdefault(metric, 0)

            if num_errors == 0:
                # whole row correct
                cvi_correct_counts[metric] += 1
            # else:
            #     if metric == "DBCV (↑)" or metric == "AD-idea (↑)" or metric == "CDbw (↑)":
            #         print(metric, file)

            # accumulate total * count
            cvi_error_counts[metric] += num_errors

    summary = pd.DataFrame({
        f"Correct evaluations (out of {len(glob.glob(FOLDER))})": pd.Series(cvi_correct_counts),
        f"Errors (out of {5 * len(glob.glob(FOLDER))})": pd.Series(cvi_error_counts)
    })

    summary.to_csv(FOLDER_RESULTS_LABELSETS + "/.summary.csv")
    print(summary)
