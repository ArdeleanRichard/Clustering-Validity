...
def plot_analysis(cvi_str, measure_str, measure_arr, cvi_tl_arr, cvi_hl_arr, chosen_idx, datasets, save_filename):
    ...
    X_plot, horiz_labels, true_labels, y_mid, val, measure_val = datasets[chosen_idx]
    ...
    axes[0].plot(measure_arr, cvi_hl_arr, marker='s', linewidth=2, label=f'{cvi_name_full} (horizontal split, 2 labels)')
    axes[0].axvline(x=measure_arr[chosen_idx], color='gray', linestyle='--', alpha=0.6)
    axes[0].text(measure_arr[chosen_idx], np.nanmax([np.nanmax(cvi_tl_arr), np.nanmax(cvi_hl_arr)]) * 0.9,
    ...
    axes[1].set_title(
        f'Horizontal split labels (HL) \n{val_name}={val}, {measure_name_acronym}={measure_val:.2f}\n'
        f'{cvi_name_full} (HL)={cvi_hl_arr[chosen_idx]:.3f} vs {cvi_name_full} (TL)={cvi_tl_arr[chosen_idx]:.3f}',
...
def analyze_measure(
    ...
    for X, labels_true, val in datasets:
        ...
        if len(np.unique(labels_h)) < 2:
            ...
            try:
                cvi_horiz = silhouette_score(X, labels_h)
            except Exception:
                cvi_horiz = np.nan
        cvi_hl_list.append(cvi_horiz)

        datasets_infos.append((X, labels_h, labels_true, y_mid, val, measure_score))

    measure_arr = np.asarray(measure_list)
    cvi_tl_arr = np.asarray(cvi_tl_list)
    cvi_hl_arr = np.asarray(cvi_hl_list)

    # pick example where horizontal > true, otherwise pick the max horizontal
    better_idx = np.where(cvi_hl_arr > cvi_tl_arr)[0]
    chosen_idx = int(better_idx[0]) if better_idx.size > 0 else int(np.nanargmax(cvi_hl_arr))
    ...
    plot_analysis(cvi_str,
                  ...
                  save_filename=f"{save_filename}_{measure_str}_{cvi_str}")


    return measure_arr, cvi_tl_arr, cvi_hl_arr
