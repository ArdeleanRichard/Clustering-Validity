...
class gSym:
	...
	def Sym(self, **kwargs):
		...
		for k in set(self.labels):
			# Members of Cluster K
			idx_k = [idx for idx, cx in enumerate(self.labels) if cx == k]
			...
			# Get other clusters
			alt_clusters = list(set(self.labels) ^ {k})

			# Get euclidean / regular distance measure between cluster centroids
			for l in alt_clusters:
				idx_l = [idx for idx, cx in enumerate(self.labels) if cx == l]
	...
	def Sym33(self, **kwargs):
		...
		for k in set(self.labels):
			...
			for j in alt_clusters:
				idx_j = [idx for idx, cx in enumerate(self.labels) if cx == j]
