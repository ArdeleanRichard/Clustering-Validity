...
def main_compare_performance(ns=[500, 1000, 2000, 5000, 10000, 15000, 20000], d=2):
    ...
    for n in ns:
        ...
        time_s = time() - start
        print(f"  AD Silhouette [k={k}]: {score:.4f} in {time_s:.3f}s")

        # Davies-Bouldin score
        start = time()
        score = ad_davies_bouldin_score(X, labels, n_neighbors=k)
        time_s = time() - start
        print(f"  AD Davies-Bouldin [k={k}]: {score:.4f} in {time_s:.3f}s")

        # Calinski-Harabasz score
        start = time()
        score = ad_calinski_harabasz_score(X, labels, n_neighbors=k)
        time_s = time() - start
        print(f"  AD Calinski-Harabasz [k={k}]: {score:.4f} in {time_s:.3f}s")

        # Calinski-Harabasz score
        start = time()
        score = arboris_index(X, labels, n_neighbors=k)
        time_s = time() - start
        print(f"  Arboris [k={k}]: {score:.4f} in {time_s:.3f}s")
